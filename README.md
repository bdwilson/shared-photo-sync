...
synced.
7.  **Local album notes**:
    *   Albums nested inside folders are included — this covers albums migrated
    from iPhoto, such as the events in an "iPhoto Events" folder, each of which
    syncs as its own album.
...
**Exclude albums by name pattern** (e.g. skip every album whose name begins with `-`):
```bash
python3 sync_albums.py --all --exclude="-*"
```

**Sync only the albums matching a pattern** (e.g. only the albums whose name begins with `-`):
```bash
