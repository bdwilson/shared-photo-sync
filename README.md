...
Google Photos. By default it syncs <b>Apple Shared Albums</b> — including any
Shared Albums that you're a member of, not just the ones you created — and it
can optionally sync your <b>regular (local) albums</b> as well via the
`--albums` option. It is designed to handle iCloud-optimized libraries by
...
6.  **Album types**: By default the tool syncs only Shared Albums. Use
`--albums local` or `--albums all` to include regular albums you created in
Photos. Smart albums (auto-generated by Photos) are never synced.
7.  **Local album notes**:
...
**Sync both shared and local albums:**
```bash
...
2.  **Scanning**: The script scans your local Apple Photos library for the selected album types (shared albums by default; see `--albums`).
